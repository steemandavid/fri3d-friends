...
| ~~A4~~ | ⛔ **DROPPED 2026-08-10 — no inline USB power meter available, and none obtainable before camp.** The five §8.7 scenarios cannot be measured, so **§8.7 stays datasheet arithmetic and must not be quoted as measured**: the runtime figures, the five-scenario table and the lever-4 battery ladder thresholds are all estimates. **This removes a Phase 6 sign-off criterion rather than satisfying it.** Mitigations that need no meter and are worth doing instead: (a) `BatteryManager.get_battery_percentage()` already rides every §9.3 heartbeat, so the admin battery histogram gives a **fleet-wide empirical drain curve** across a real camp day — coarser than a meter but from 700 real badges rather than one bench unit; (b) run one badge to flat on battery and record wall-clock hours against that curve. Neither validates the per-lever attribution the ladder assumes; that stays a known unknown. |
...
| ~~**A6**~~ | ✅ **RESOLVED 2026-08-11 — and the first answer was WRONG; this supersedes it.** The real finding: **lvgl here is byte-per-glyph AND the fonts carry Latin-1.** Measured on badge with `chr(0xEB)`: **10 px, identical to an ASCII 'e'** (a genuine missing glyph, 0xFF, is 9 px). The bundled `montserrat_name.ttf` cmap has 197 codepoints including é ë ï ü ç ß (parsed on the host). So **accents DO render — what does not render is UTF-8**, which draws one box per byte. My first measurement concluded the opposite because it fed the badge UTF-8 (and a source-encoded literal) rather than a Latin-1 byte. ⚠️ **Three probes give false answers**, all tried: `get_glyph_width()` returns a per-codepoint fallback; a UTF-8 literal measures as boxes; and a source `"ë"` measures 0. Use `lv.text_get_size` on **`chr(0xEB)`** with `chr(0xFF)` as the missing-glyph control. **Consequence (0.11.29):** the split is per-render-site — the badge's OWN name label uses `to_latin1()` and shows `Renée` correctly (the README's "never strip accents from names" rule was right all along), while the **HSNT beacon still folds to ASCII**, because `parse_payload` does `decode("utf-8")` and on this build that **raises** on a Latin-1 byte (the `errors` argument is unsupported), leaving the receiver with an empty name. |
