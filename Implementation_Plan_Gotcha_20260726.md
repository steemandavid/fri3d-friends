...
| ~~**A6**~~ | ✅ **RESOLVED 2026-08-10 — NO-GO, and for a more basic reason than font coverage.** **lvgl 9.4.0 on this build is in ASCII text-encoding mode: it renders one glyph box PER BYTE and never decodes UTF-8.** Measured on bac8 with `lv.text_get_size`: a 2-byte UTF-8 `é` is exactly two fallback boxes wide, a 3-byte `€` three, a 4-byte emoji four (9/18/27/46 px against a 9 px box). Font coverage is irrelevant — **no** multi-byte character can render. This build's MicroPython is byte-oriented in source too (`"e"+"ë"+"e"` is 3 bytes, not 4). **The §8.9 ASCII-only rule stands.** ⚠️ Two probes give FALSE answers here, both tried: `get_glyph_width()` returns a per-codepoint fallback (12) for missing glyphs, so accents *look* present; and measuring a bare high byte gives width 0, so they then *look* absent. Only `lv.text_get_size()` on proper UTF-8 **with CJK/Hebrew controls** discriminates. **Consequence fixed in 0.11.25:** an accented name was mojibake on the badge and in every peer's nearby-list; `ble_proximity.fold_ascii()` now folds at the render boundary (`build_payload` + the nametag label), while `sanitize_config` keeps storage verbatim — `contacts.json` is irreplaceable (§8.10.4) and the web card renders UTF-8 fine. |
